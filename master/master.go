...
func main() {
	...
	//Controlla che gli indirizzi siano legali attravesro una regex(ip, localhost oppure nome ([a-zA-Z]+_([a-zA-Z]+))
...
func starWorkers(array []string) {
	...
	for key, token := range array {
		...
		go func() {
			defer wg.Done()
			fmt.Printf("Calling Mapping operation on host: %s\n", hosts[key])
